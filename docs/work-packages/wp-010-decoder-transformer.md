...
Assemble and smoke-train the tiny decoder-only model.

## Learning outcomes

Explain and implement decoder-only transformer in an evidence-backed way.
...
Implement and document decoder-only transformer with tests, experiments, and guide updates appropriate to the work package stage.
